...
import { CONTEST_LABELS, recalcTeam } from '@/utils/summerAltScoring'
import type { TeamWithRank } from '@/utils/summerAltScoring'
...
const sortedTeams = computed<TeamWithRank[]>(() => {
  ...
  const recalculated = d.teams.map((t: any) => recalcTeam(t))

  const sorted = recalculated.sort((a: TeamWithRank, b: TeamWithRank) => b.team_total - a.team_total)

  // 计算并列排名
  let currentRank = 1
  let prevScore = -1

  return sorted.map((t: TeamWithRank, i: number) => {
...
function getContestData(team: TeamWithRank, contestIndex: number) {
  if (!team.contests || !team.contests[contestIndex]) {
    return { solved: '-', rank: '-', score: '-', isBest: false, excused: false, absent: false, imputed: false, violation: false }
  }

  const contest = team.contests[contestIndex]

  // 违规：成绩作废按 0 分计算，保留原有过题数和排名
  if (contest._violation) {
    return {
  ...
  if (contest.absent) {
    return { solved: '-', rank: '-', score: 0, isBest: contest.isBest || false, excused: false, absent: true, imputed: false, violation: false }
  }
...
function getScoreColorClass(score: string | number, isImputed: boolean = false): string {
  if (score === '-') return ''
...
<template>
  <div class="summer-scores">
    <div class="page-header">
      ...
      <p class="page-desc">
        取最好 <strong>16 场</strong>（80%）的平均成绩计入总成绩<br>
        淡灰色 = 未计入成绩的场次<br>
        淡红色 = 违规，成绩作废<br>
        淡橙色 = 因公事务缺席（估算分）
      </p>
            ...
            <tbody>
              <tr v-for="team in sortedTeams" :key="team.name_cn">
                ...
                <td
                  ...
                  class="col-contest"
                  :class="{
                    'not-best': !getContestData(team, ci).isBest && !getContestData(team, ci).excused && !getContestData(team, ci).violation && (getContestData(team, ci).solved !== '-' || getContestData(team, ci).absent),
                    'col-excused': getContestData(team, ci).excused,
                    'col-violation': getContestData(team, ci).violation,
                  }"
                >
                  <template v-if="getContestData(team, ci).violation">
                    ...
                    <span
                      class="sub-item score-violation"
                      title="违规，成绩作废"
                    >
                      0.00
                    </span>
                  </template>
                  <template v-else-if="getContestData(team, ci).imputed">
                    <span class="sub-item">-</span>
                    <span class="sub-item">-</span>
                    <span
                      class="sub-item score-imputed-cell"
                      :class="getScoreColorClass(getContestData(team, ci).score, true)"
                      :title="`估算分：该队其余场次最高5场平均 = ${formatContestValue(getContestData(team, ci).score, true)}`"
                    >
        ...
        <div class="rules-grid">
          <div class="rule-card rule-card--general">
            ...
            <ul class="rule-list">
              ...
              <li>
                <strong style="background:#fff3e0;padding:2px 6px;border-radius:3px;">因公出差</strong>场次的得分
...
.team-score-table tbody tr:hover .col-violation {
  background: #fecaca;
}
