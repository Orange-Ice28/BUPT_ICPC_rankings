...
OUTPUT_FILE = os.path.join(DATA_DIR, "score_result_inner.xlsx")
...
def calc_contest_score_inner(solved, rank, baseline, total_people):
    if baseline == 0 or total_people == 0:
        return 0.0
    score = (solved / baseline) * (total_people - rank + 1) / total_people * 100
    if score < 0 or score > 100:
        return 0.0
    return score


def calc_inner_ranks(rank_data, team_member_names, contest_idx):
    entries = []
    for item in rank_data:
        if item["name"] in team_member_names and item["contests"][contest_idx]["solved"] > 0:
            entries.append((item["name"], item["contests"][contest_idx]["rank"]))
    ...
    return total_people, name_to_rank
...
def main():
    ...
    total_people_per_contest = []
    inner_rank_maps = []
    for i in range(NUM_CONTESTS):
        tp, rank_map = calc_inner_ranks(rank_data, team_member_names, i)
        total_people_per_contest.append(tp)
        inner_rank_maps.append(rank_map)
    ...
    for item in rank_data:
        ...
        for i, c in enumerate(item["contests"]):
            inner_rank = inner_rank_maps[i].get(item["name"], 0)
            s = calc_contest_score_inner(c["solved"], inner_rank, baselines[i], total_people_per_contest[i])
            scores.append(s)
            details.append({
                "solved": c["solved"],
                "rank": inner_rank,
    ...
    for tid in in_team:
        ...
        row_data = [rank_counter, tid, item["name"], personal_scores[tid]]
        rank_counter += 1
        ...
        for ci in range(NUM_CONTESTS):
            if ci not in best:
                col_start = 5 + ci * 3
                for dc in range(3):
                    ws1.cell(row=row_idx, column=col_start + dc).fill = GRAY_FILL

    if not_in_team:
        ws1.append([])
        for tid in not_in_team:
            item = team_id_to_data[tid]
            row_data = [rank_counter, tid, item["name"], personal_scores[tid]]
            rank_counter += 1
            for d in personal_contest_details[tid]:
    ...
    print("\n各场总人数:")
    for i, tp in enumerate(total_people_per_contest, 1):
        print(f"  第{i}场: {tp}")
